  ...
  async run({ page, shared, params, logger }) {
    ...
    for (let i = 0; i < draws.length; i++) {
      pool.push(async () => {
        ...
        const headers = {
          'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36', // 請根據需要更新為最新版瀏覽器的 User-Agent
          'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9',
          'Accept-Language': 'zh-TW,zh;q=0.9,en-US;q=0.8,en;q=0.7', // 根據您的語言偏好設定
          'Referer': 'https://fuli.gamer.com.tw/',  // 根據實際情況設定
        ...
        for (let attempts = 1; attempts <= MAX_ATTEMPTS; attempts++) {
          try {
            await task_page.setExtraHTTPHeaders(headers); // 設定 Headers
    ...
    if (recaptcha.process === true) {
        ...
        if (recaptcha_frame_width !== "100%") {
          logger.log("需要處理 reCAPTCHA");
            try {
              // 優化 solve 呼叫，增加 delay 選項
              await timeout_promise(solve(page, { delay: 1000 }), 30000); // 設定更長的超時時間，並增加延遲
            } catch (err) {
