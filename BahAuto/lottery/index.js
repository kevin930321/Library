                       ...
                       try{
                          ...
                          data: "token=" + encodeURIComponent(csrfToken) + "&area=item&sn=" + encodeURIComponent(snValue)
                            ...
                            logger.error("發送已看廣告請求時發生錯誤:", error);
                              ...
                              //這裡點擊之後，如果沒廣告或是你已經看過， 就會進入結算頁面， 因此不需要確認是否跳出確認彈窗， 進入結算頁面的前提一定是符合條件(沒問題)的狀況
                    break
                      }
               await  task_page.click("text=看廣告免費兌換").catch(()=>logger.error("跳過廣告按鈕失效， 請檢查流程。"));

            // --- 跳過廣告 的核心邏輯結束---
            await Promise.all([
               task_page.waitForNavigation().catch(()=>{}
               ),
