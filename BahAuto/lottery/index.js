                        ...
                        for (let retried = 1; retried <= CHANGING_RETRY; retried++) {
                         ...
                         if (!(await adButtonLocator.isVisible())) {
                            ...
                            const snValue = urlParams.get('sn');
                                ...
                                const data = JSON.parse(await response.text());
                                 ...
                                 if (data.data && data.data.finished === 1) {
                                     logger.info("你已經看過或跳過廣告!");
                                      ...
                                      const check_ad_data = JSON.parse(await check_ad_response.text());

                                       if(check_ad_data.data && check_ad_data.data.finished !== 1){
                                          logger.warn("伺服器端廣告狀態未同步，嘗試重新請求廣告流程");
                                          break;
                                       }
                                       break;
                                }
                             ...
                             const tokenResponse = await task_page.request.get("https://fuli.gamer.com.tw/ajax/getCSRFToken.php?_=1702883537159");
                             const csrfToken = (await tokenResponse.text()).trim();
                           let finishAdResponse;

                                  try {
                                   finishAdResponse =  await task_page.request.post('https://fuli.gamer.com.tw/ajax/finish_ad.php', {
                                          headers: {
                         ...
                         if(!finishAdResponse.ok){
                              logger.error("發送已看廣告請求時發生錯誤, 伺服器回應狀態:", finishAdResponse.status);
                                  break;
                            }
                                   const responseData = await finishAdResponse.text()

                                    if (responseData !== "success"){
                                          logger.error("伺服器沒有正確處理已看廣告請求")
                                          break;
                                     }
