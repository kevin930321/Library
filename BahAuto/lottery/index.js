    ...
    for (let i = 0; i < draws.length; i++) {
      pool.push(async () => {
        ...
        for (let attempts = 1; attempts <= MAX_ATTEMPTS; attempts++) {
          try {
            await task_page.goto(link, { waitUntil: 'domcontentloaded'}); // 這裡新增等待事件，避免資源尚未載入完成。
                            ...
                            try {
                                  await task_page.request.post("https://fuli.gamer.com.tw/ajax/answer_question.php",{
                                     ...
                                     await task_page.reload();
                                        ...
                                        logger.error("post 回答問題時發生錯誤")
                                      continue
                                          }
                       logger.info("成功回答問題！")
                     await task_page.waitForLoadState('networkidle'); // 等待問題重整完成
