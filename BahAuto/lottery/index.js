        ...
        for (let i = 0; i < draws.length; i++) {
            pool.push(async () => {
                ...
                for (let attempts = 1; attempts <= MAX_ATTEMPTS; attempts++) {
                    try {
                        await task_page.waitForTimeout(Math.random() * 1000 + 500);
                        await task_page.goto(link);
                        await task_page.waitForSelector("#BH-master > .BH-lbox.fuli-pbox h1");
                        await task_page.waitForTimeout(Math.random() * 1000 + 100);
                        ...
                        for (let retried = 1; retried <= CHANGING_RETRY; retried++) {
                            try {
                                ...
                                const wait_time = Math.min(60000, (Math.random() + 1) * 1000 * Math.pow(2, retried));
                                logger.warn(`嘗試重試 ( ${retried}/${CHANGING_RETRY}) 次， 稍後 ${Math.floor(wait_time / 1000)} 秒再試`);
                                await task_page.waitForTimeout(wait_time)
                            }
                        }
                        await task_page.waitForTimeout(Math.random() * 1000 + 500);
                        await Promise.all([
                        ...
                        await task_page.waitForTimeout(Math.random() * 1500 + 1000);
                        ...
                        const wait_time = Math.min(60000, (Math.random() + 1) * 1000 * Math.pow(2, attempts));
                        logger.warn(`嘗試重試 ( ${attempts}/${MAX_ATTEMPTS}) 次， 稍後 ${Math.floor(wait_time / 1000)} 秒再試`);
                        await task_page.waitForTimeout(wait_time)
                        logger.error("!", err);
                    }
                }
